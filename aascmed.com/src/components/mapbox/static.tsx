...
export const StaticMap: React.FC<MapBoxProps> = (props) => {
    ...
    return (
        <div className={props.className} css={css(stylesStaticMap)}>
            <noscript>
                <style dangerouslySetInnerHTML={{ __html: `#${id.replace(":", "\u{003A}")} {display: none;` }} />
                <Global styles={css``} />
            ...
            <div id={id} className={`lazy-container`}>
                {
                    ...
                    <img ref={imageRef} onLoad={unBlurImage} className={`map ${loaded}`} src={image.src} srcSet={image.srcset} alt={props.alt} sizes={image.sizes} />
                }
                {
